...
class Xinjiang extends MY_Controller
{
    private $_info_url = 'http://gsxt.xjaic.gov.cn:7001/ztxy.do';
    private $_getdata_url = '';
    private $_verify_url = '';
    private $_checkcode_url = '';
    protected $_cookies = 'xinjiang.cookie';
    function __construct()
    {
        parent::__construct();
        $this->cookie_jar .= $this->_cookies;
        $this->_getdata_url = 'http://gsxt.xjaic.gov.cn:7001/ztxy.do?method=list&djjg=&random=1474599310521';
        $this->_verify_url = 'http://gsxt.xjaic.gov.cn:7001/ztxy.do?method=createYzm&dt='.time().'&random='.time();
    }
    ...
    public function serchlist(){
        ...
        $content = iconv('GBK','utf-8',$content);//转码
        ...
        if(empty($date)){
            exit("<a href='" . site_url('collection/xinjiang') . "'>验证码不正确,请返回!</a>");
        } else {
            $data['list'] = $date;
            $this->_render('xinjiang/serchlist',$data);
        }
    }
    ...
    public function detail()
    {
        ...
        $this->_render('xinjiang/detail',$data);
...
//                make_html($path,iconv('utf-8','gb2312',$ob_content['content']));
            ...
            //保存sql
            save2db( array('id'=> get_uid(),'company_name'=>trim($title),'page_path'=>base_url("html/".$path),'province'=>' 新疆'));
        }
    }

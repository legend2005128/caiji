...
class Qinghai extends MY_Controller
{
    private $_base_url = 'http://gsxt.qhaic.gov.cn:83';
    private $_getdata_url = 'http://gsxt.qhaic.gov.cn:83/searchList.jspx';
    private $_verify_url = '';
    private $_checkcode_url = 'http://gsxt.qhaic.gov.cn:83/checkCheckNo.jspx';
    protected $_cookies = 'qinghai.cookie';
    function __construct()
    {
        parent::__construct();
        $this->cookie_jar .= $this->_cookies;
        $this->_verify_url = 'http://gsxt.qhaic.gov.cn:83/validateCode.jspx?type=0&id='.time();
//        $this->_verify_url = 'http://gsxt.qhaic.gov.cn:83/validateCode.jspx?type=0&id=0.869065668061119';
    }
    ...
    public function serchlist(){
        ...
        if( strpos($ck_2,'false')!==false ){
            exit("<a href='" . site_url('collection/qinghai') . "'>验证码不正确,请返回!</a>");
        }
        $post_data = array(
            'checkNo' => $code,
            'entName' => $name,
        ...
        $data['list'] = $date;
        $this->_render('qinghai/serchlist',$data);
    }
    ...
    public function detail()
    {
        ...
        $this->_render('qinghai/detail',$data);
        //保存html
...
//                make_html($path,iconv('utf-8','gb2312',$ob_content['content']));
            ...
            //保存sql
            save2db( array('id'=> get_uid(),'company_name'=>trim($title),'page_path'=>base_url("html/".$path),'province'=>'青海省'));
        }
    }

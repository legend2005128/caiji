...
class heilongjiang extends MY_Controller
{
    private $_origin = "http://gsxt.hljaic.gov.cn";
    private $_list_url;
    private $_yzm_url;
    private $_yzm_ck_url;
    private $_entname_url = 'http://gsxt.hljaic.gov.cn/checkFilterKey.jspx';
    private $_content_url ='http://gsxt.hljaic.gov.cn/businessPublicity.jspx?id=';

    protected $_name = '';//公司名
    protected $_vc = '';//验证码
    protected $_cookies = 'heilongjiang.cookie';
    public $render_default = 'heilongjiang/';

    function __construct()
    {
        parent::__construct();
        //验证码地址
        $this->_yzm_url = $this->_origin . '/validateCode.jspx';
        //验证码校验url
...
//        $ck_1 = curl_post($this->_entname_url, $post_ck_num, $this->_header, $this->cookie_jar);
        ...
        if( strpos($ck_2,'false')!==false ){
            exit("<a href='" . site_url('collection/heilongjiang') . "'>验证码问题,请返回!</a>");
        }
        //3查询数据
    ...
    public function set_list_data($contents)
    {
        ...
        if ($contents && strlen($contents)) {
            ...
            if (!$arr || !@$arr[1]) {
                exit("<a href='" . site_url('collection/heilongjiang') . "'>暂无数据重新搜索</a>");
            }
    ...
    public function contents()
    {
        ...
        $data['base'] =  str_replace(array('<br>','详情','<div id="invDiv">','</div>'),'', $arr_base[2] );
        preg_match('#<div id="altDiv">(.*)</div>#iUs', $content,$arr_gd);
         $data['base_bg'] = str_replace('详情', '', $arr_gd[1]);
        //2.行政处罚
        ...
        $data['jyyc'] =  str_replace(array('<br>','详情','<div id="excDiv">','</div>'),'', $arr_jyyc[2] );
        //4.严重违法
        ...
        if(! get_one_item($post_arr['company_name'])){
            ...
            //保存sql
            save2db( array('id'=> get_uid(),'company_name'=>$post_arr['company_name'],'page_path'=>base_url("html/".$path),'province'=>'黑龙江省'));
        }
    }
}

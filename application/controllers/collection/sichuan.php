...
class Sichuan extends MY_Controller
{
    private $_info_url = 'http://gsxt.scaic.gov.cn/ztxy.do';
    private $_getdata_url = '';
    private $_verify_url = '';
//    private $_checkcode_url = 'http://gsxt.ngsh.gov.cn/ECPS/qyxxgsAction_checkVerificationCode.action';
    protected $_cookies = 'sichuan.cookie';
    function __construct()
    {
        parent::__construct();
        $this->cookie_jar .= $this->_cookies;
        $this->_getdata_url = 'http://gsxt.scaic.gov.cn/ztxy.do?method=list&djjg=&random=1474506982113&yourIp='.getIP();
        $this->_verify_url = 'http://gsxt.scaic.gov.cn/ztxy.do?method=createYzm3&dt=1474507756718&random='.time();
    }
    ...
    public function serchlist(){
        ...
        $data['list'] = $date;
        $this->_render('sichuan/serchlist',$data);
    }
...
//                make_html($path,iconv('utf-8','gb2312',$ob_content['content']));
            ...
            //保存sql
            save2db( array('id'=> get_uid(),'company_name'=>trim($title),'page_path'=>base_url("html/".$path),'province'=>'四川省'));
        }
    }

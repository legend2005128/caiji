...
class Guangxi extends MY_Controller
{
    private $_base_url = 'http://gxqyxygs.gov.cn';
    private $_getdata_url = 'http://gxqyxygs.gov.cn/searchList.jspx';
    private $_verify_url = '';
    private $_checkcode_url = "http://gxqyxygs.gov.cn/checkCheckNo.jspx";
    protected $_name = '';//公司名
    protected $_code = '';//验证码
    protected $_cookies = 'guangxi.cookie';
    function __construct()
    {
        require_once (APPPATH.'/libraries/phpQuery/phpQuery.php');
        parent::__construct();
        $this->cookie_jar .= $this->_cookies;
        $this->_verify_url = 'http://gxqyxygs.gov.cn/validateCode.jspx?type=1&id='.time();
    }
    ...
    public function serchlist(){
        ...
        if( strpos($ck_2,'false')!==false ){
            exit("<a href='" . site_url('collection/guangxi') . "'>验证码不正确,请返回!</a>");
        }
        $post_data = array(
            'checkNo' => $code,
            'entName' => $name,
        ...
        $data['list'] = $date;
        $this->_render('guangxi/serchlist',$data);
    }
    ...
    public function detail()
    {
        ...
        $this->_render('guangxi/detail',$data);
        //保存html
...
//                make_html($path,iconv('utf-8','gb2312',$ob_content['content']));
            ...
            //保存sql
            save2db( array('id'=> get_uid(),'company_name'=>trim($title),'page_path'=>base_url("html/".$path),'province'=>'广西省'));
        }
    }
